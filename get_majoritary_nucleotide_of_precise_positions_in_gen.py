...
#     				Ce script permet de sortir le nucléotide majoritaire pour des positions données d'un fichier GEN :
...
parser = argparse.ArgumentParser(description= 'Ce script permet de sortir le nucléotide majoritaire pour des positions données d un fichier GEN :')
...
tmp=open(output,"w")
tmp.write("Contig"+"\t"+"Position"+"\t"+"Nb_homo"+"\t"+"Nb_hetero"+"\t"+"Pourc_hetero"+"\t"+"Nb_missing"+"\t"+"homo_maj"+"\t"+"nb_A"+"\t"+"nb_C"+"\t"+"nb_G"+"\t"+"nb_T"+"\n")
...
for line in open(gen) :
	...
	#Sinon je me balade a la recherche de ma position
		...
		if str(num) in liste:
			...
			tmp.write(contig+"\t"+str(num)+"\t"+str(nb_homo)+"\t"+str(nb_hetero)+"\t"+str(pourc_hetero)+"\t"+str(nb_missing)+ "\t"+str(nucl)+"\t"+str(nb_A)+"\t"+str(nb_C)+"\t"+str(nb_G)+"\t"+str(nb_T)+"\n")
